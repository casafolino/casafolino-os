...
2. Config value: `node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  if [ "$HAS_STATUS" = "valid" ]; then
    ...
    if [ "$COMMIT_DOCS" = "true" ]; then
      node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit \
