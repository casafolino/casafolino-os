...
@/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/agent-contracts.md
@/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/context-budget.md
@/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/gates.md
</required_reading>
...
```bash
INIT=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/planner-antipatterns.md
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/antoniofolino/casafolino-os/.claude/get-shit-done/workflows/execute-plan.md` **inline**
...
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_execution --default false 2>/dev/null)
CROSS_AI_CMD=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_command --default "" 2>/dev/null)
CROSS_AI_TIMEOUT=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.cross_ai_timeout --default 300 2>/dev/null)
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       <execution_context>
       @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/workflows/execute-plan.md
       @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/templates/summary.md
       @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/checkpoints.md
       @/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Users/antoniofolino/casafolino-os/.claude/get-shit-done/references/executor-examples.md'}
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get commit_docs 2>/dev/null || echo "true")
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
   elif [ "${TEST_EXIT}" -eq 124 ]; then
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
TDD_MODE=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode --default false 2>/dev/null)
```
...
```bash
POST_PLAN_INDEX=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
PARENT_INFO=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
**Check config gate:**
```bash
GL_ENABLED=$(node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
```bash
node "/Users/antoniofolino/casafolino-os/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/Users/antoniofolino/casafolino-os/.claude/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
