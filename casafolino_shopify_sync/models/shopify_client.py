...
class CasaFolinoShopifyClient(models.AbstractModel):
    ...
    def get_available_quantity(self, inventory_item_id, location_id):
        query = """
            query InventoryQuantity($inventoryItemId: ID!, $locationId: ID!) {
        ...
        available = [item for item in quantities if item.get("name") == "available"]
        return int((available or [{"quantity": 0}])[0].get("quantity") or 0)

    def set_available_quantity(self, inventory_item_id, location_id, quantity, reference):
        current_quantity = self.get_available_quantity(inventory_item_id, location_id)
        mutation = """
