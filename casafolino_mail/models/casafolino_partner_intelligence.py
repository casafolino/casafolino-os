...
class CasafolinoPartnerIntelligence(models.Model):
    ...
    hotness_tier = fields.Selection([
        ('hot', 'Hot'), ('warm', 'Warm'), ('active', 'Active'),
        ('cold', 'Cold'), ('dormant', 'Dormant'),
    ...
    def _compute_component_revenue(self, partner):
        """Revenue score 0-100 (log scale, 12-month invoices)."""
    ...
    def _compute_component_activity(self, partner):
        """Activity score: 90-day event count (emails + orders + activities)."""
        cutoff = fields.Datetime.now() - timedelta(days=90)
        email_count = self.env['casafolino.mail.message'].search_count([
            ('partner_id', '=', partner.id),
        ...
        order_count = self.env['sale.order'].sudo().search_count([
            ('partner_id', '=', partner.id),
            ('date_order', '>=', cutoff),
        ]) if 'sale.order' in self.env else 0
        activity_count = self.env['mail.activity'].search_count([
            ('res_model', '=', 'res.partner'),
            ('res_id', '=', partner.id),
            ('create_date', '>=', cutoff),
        ])
        total = email_count + order_count + activity_count
        if total == 0:
            return 0
        if total <= 5:
            return 30
    ...
    def _compute_component_pipeline(self, partner):
        """Pipeline score: max(stage_weight * probability) on open leads."""
