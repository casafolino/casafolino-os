...
def _ensure_standby_cron(cr):
    ...
    # Step 2: create ir.cron (Odoo 18: user_id NOT NULL, no numbercall)
    cr.execute("""
        INSERT INTO ir_cron (
            cron_name, ir_actions_server_id, user_id, active,
            interval_number, interval_type, priority, nextcall,
            create_uid, write_uid, create_date, write_date
        ) VALUES (
            'CasaFolino: Auto-Standby Lead inattivi',
            %s, 1, true,
            1, 'days', 5, NOW() + interval '1 day',
            1, 1, NOW(), NOW()
        )
    """, (server_id,))
